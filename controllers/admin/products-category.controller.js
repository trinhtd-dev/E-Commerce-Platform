...
module.exports.index = async (req, res) => {
  ...
  records = buildCategoryTree(records);
  ...
  const newRecord = records.slice(
    objectPagination.skip,
    objectPagination.skip + objectPagination.limit
  );
  ...
  switch (req.body.type) {
    case "active":
      try {
        ...
        req.flash(
          "success",
          `Change status for ${ids.length} product categories successfully`
        );
      } catch (error) {
        req.flash(
          "error",
          `Change status for ${ids.length} product categories failed`
        );
      }
      break;
    ...
    case "delete":
      try {
        ...
        req.flash(
          "success",
          `Deleted ${ids.length} product categories successfully`
        );
      } catch (error) {
    ...
    case "change-position":
      try {
        ...
        req.flash(
          "success",
          `Change position for ${ids.length} product categories successfully`
        );
      } catch (error) {
        req.flash(
          "error",
          `Change position for ${ids.length} product categories failed`
        );
      }
      break;
  }
